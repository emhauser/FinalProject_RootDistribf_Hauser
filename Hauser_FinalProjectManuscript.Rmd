...
#Discussion
Recent studies suggest that plants allocate their C resources to nutrient acquisition strategies that maximize the nutrient return on C investment [@LynchandHo2005]. In shallow soil horizons, the relatively high abundance of soil organic matter suggests that SOM decomposition may be a readily available nutrient source, while weathering of mineral-bound nutrients may be the least C-expensive if roots can reach sufficient depth [@CITE!!!]. Assuming that nutrient acquisition is a primary driver of root growth, we might expect to see roots tracking SOM pools in upper soil horizons and mineral pools in deeper horizons where unweathered minerals are more readily available.  

Results of this study suggest that deep roots may indeed be tracking mineral-bound nutrients where they are relatively more available. Throughout the soil profile, we observed strong correlations between roots and soil organic carbon (SOC), which is a proxy for organic matter (SOM). Although such strong correlations are expected because roots themselves comprise part of SOC, the correlation is tightest in shallow soils (Fig. 3), where roots have the most access to organically-derived nutrient pools. This correlation weakens deeper in the soil profile, suggesting that the relationship between root distribution and SOC dissolves where roots are growing deep. Therefore, shallow roots may track readily available SOM-derived nutrients, but deep roots may be growing for a different resource.

Of course because roots track SOM, se relationship between root abund and Po but when deepr that relationship actually reverses, suggesing that deep roots are less prevalent surrounding Po. In deep soils roots actually track Pi. 
