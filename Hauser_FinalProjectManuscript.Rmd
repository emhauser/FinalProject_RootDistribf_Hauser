...
```{r prelim, echo=FALSE, fig.cap= "Figure 2. A) Correlation matrix showing relationships between root distributions, P-fractions, Ca, Mg, and K in Hardwood forests at the Calhoun Critical Zone Observatory, including Pearson's correlation coefficient. B) Plots of those same data relationships for Pine forests at Calhoun." out.width='.49\\linewidth', fig.width=3, fig.height=3,fig.show='hold',fig.align='center'}
SoilRainbowStats(HWSoilDat, 1, 175, 17, 25)
SoilRainbowStats(PineSoilDat, 1, 175, 17, 25)
```


Upon closer analysis, we observe opposing trends between roots and available P_o_ vs. roots and available P_g_ in shallow vs. deep hardwood soils (Fig. 3). 

We also observe depth-dependent relationships between roots and Ca, as well as roots and K in undisturbed hardwoods (Figs. 4 & 5). Trends in pine forests are less distinct, but appear to be stronger in surface soils than deep soils (Figs. 4 & 5). 

<!--A chunk to make P Figure-->
```{r P figs, include=FALSE}
...
In pine forests, we observed trends between root distribution and multiple P fractions that were similar to those same trends in much deeper hardwood forests. Shown here for residual P, deep roots in hardwood forests track residual P to a comparable extent as shallower roots in pine forests (Fig. 6).

<!--A chunk for depth-translation of P relationships between HW and Pine -->
```{r PdepthShift, include=F}
```
